...
$(window).scroll(function () {
	...
	if (scroll >= 100) {
		$(".mobile-navbar").addClass("darkHeader");
	} else {
		$(".mobile-navbar").removeClass("darkHeader");
	}
});
...
var owl_three = $('.owl-three-custom-1')
jQuery(owl_three).each(function () {
	jQuery(this).owlCarousel({
		slideSpeed: 500,
		autoPlay: 5000,
...
$('.navbar-collapse').find('.dropdown').each(function () {
	...
	if ($(this).closest('.navbar-toggle').css()) { // only on collapsed nav
		$(this).on({
			"click": function (e) {
